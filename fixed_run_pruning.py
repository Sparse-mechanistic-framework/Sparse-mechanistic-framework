...
def train_pruned_model_with_verification(model, masks, train_loader, eval_loader, 
    ...
    Returns FINAL epoch metrics, not best metrics"""
    ...
    try:
        for epoch in range(config.num_epochs):
            ...
            # Store last epoch metrics (not best!)
            final_metrics = metrics.copy()
            
            logger.info(f"Loss: {loss:.4f}, Correlation: {metrics['correlation']:.4f}, MSE: {metrics['mse']:.4f}")
    ...
    for method in config.pruning_methods:
        ...
        for sparsity in config.target_sparsities:
            ...
            try:
                ...
                final_metrics = train_pruned_model_with_verification(
                    model, masks, train_loader, eval_loader, 
                    config, trainer, logger, sparsity
                )
                
                # Verify final sparsity
                final_sparsity = trainer.calculate_sparsity(model)
                
                # Calculate retention using FINAL correlation (not best)
                ...
                all_results['methods'][method][sparsity] = {
                    ...
                    'retention': retention,
                    'final_correlation': final_metrics['correlation']  # Explicitly store final
                }
                
                logger.info(f"\nFinal Results for {method} at {sparsity:.0%}:")
                logger.info(f"  Actual sparsity: {final_sparsity:.2%}")
                logger.info(f"  Final correlation: {final_metrics['correlation']:.4f}")
                logger.info(f"  Final MSE: {final_metrics['mse']:.4f}")
